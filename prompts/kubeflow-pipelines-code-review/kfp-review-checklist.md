...
Apply every check explicit, even when file not obvious match.
...
- For run/job fields (example `plugins_input` and `plugins_output`), ensure every create and update write path persists fields consistent with same nil/NULL handling.
...
- If one-off run behavior changes, check whether recurring runs need parity; raise uncertainty explicit.
...
- In pipeline-agnostic layers, confirm no Argo-specific implementation details introduced.
- Flag cross-binary internal imports (example controller importing apiserver internals); require shared/common or API boundaries.

## Frontend, SDK, and Manifests

- For `frontend/`, verify API contract changes reflected in queries/mutations and feature-flag behavior.
